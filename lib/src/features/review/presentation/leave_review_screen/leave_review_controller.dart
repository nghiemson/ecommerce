...
class LeaveReviewController extends StateNotifier<AsyncValue<void>> {
  ...
  // this is injected so we can mock the date in the tests
  ...
  Future<void> submitReview({
    ...
    // only submit if the rating is new or has changed
    if (previousReview == null ||
      ...
      if (mounted) {
        // only set the state if the controller has not been disposed
        state = newState;
        if (!state.hasError) {
          onSuccess();
        }
